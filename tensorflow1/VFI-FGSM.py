...
import os
import pandas as pd
from filtering import median_blur, mean_blur
...
tf.flags.DEFINE_integer('P_kernel_size', 3, 'kernel size in PI.')

# tf.flags.DEFINE_integer('T_kernel_size', 3, 'kernel size in TI.')

tf.flags.DEFINE_string('sfilter', 'median', 'filter type in FI,none/median/mean/gaussian.')

tf.flags.DEFINE_string('train_model', 'Inc-v3', 'Inc-v3/Inc_v4/Res152/IncRes_v2.')

tf.flags.DEFINE_float('constant0', 1.0, 'c0--parameter of volatile step size ')
tf.flags.DEFINE_float('constant1', 1.0, 'c1--parameter of volatile step size ')
tf.flags.DEFINE_float('constant2', 1.0, 'c2--parameter of volatile step size ')

tf.flags.DEFINE_float('lam', 1.0, 'weight of auxlogits')
...
def getLoss(x, y):
    ...
    if flag == 1:
        cross_entropy += tf.losses.softmax_cross_entropy(one_hot, auxlogits)
...
def graph(x, y, i, x_max, x_min, grad, amplification):
    ...
    c1 = FLAGS.constant1
    c2 = FLAGS.constant2
    L = norm_li(x, (x_min + x_max) / 2) / num_iter
    p_alpha = c0 * alpha - c1 * alpha * L  # 1.线性函数

    # p_alpha = c2 * alpha * tf.exp((-1) * L)  # 3.指数函数

    x = x + p_alpha * tf.sign(noise)  # VI
...
def main(_):
    ...
    with tf.Graph().as_default():
        ...
        # Run computation 实例化saver对象
        s1 = tf.train.Saver(slim.get_model_variables(scope='InceptionV3'))  # 不指定具体变量就是恢复所有变量
        ...
        s7 = tf.train.Saver(slim.get_model_variables(scope='EnsAdvInceptionResnetV2'))  # 和166行scope对应
        s8 = tf.train.Saver(slim.get_model_variables(scope='resnet_v2_101'))  # 上面没写则是默认与resnet_v2.py相同
