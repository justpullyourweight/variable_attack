...
def input_diversity(FLAGS, input_tensor):
    ...
    rnd = tf.random_uniform((), FLAGS.image_width, FLAGS.image_resize, dtype=tf.int32)  ## random_uniform()生成tensor
    rescaled = tf.image.resize_images(input_tensor, [rnd, rnd], method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)  # 调整图像大小
